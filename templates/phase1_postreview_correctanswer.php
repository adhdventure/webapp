<?php
  ...
  **/
  ...
  $gdatamajor = $wpdb->get_results("SELECT SUM(points) AS studentscor, MAX(studentscor) AS LargestPrice, learning_style FROM `adhd_learning_points_system` WHERE userid ='$current_users' GROUP BY userid,learning_style ORDER BY studentscor DESC  LIMIT 1");
  foreach ($gdatamajor as $key => $vasdflue) {
